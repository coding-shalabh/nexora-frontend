...
function ConnectedAccount({ account, provider, balance, onRefresh, onDisconnect }) {
    ...
    <motion.div variants={itemVariants} className="bg-white rounded-2xl p-6 shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-semibold text-gray-900">{account.name || 'WhatsApp Business'}</h3>
      ...
      {balance !== undefined && (
        <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-green-50 to-emerald-50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Wallet className="h-4 w-4 text-green-600" />
              <span className="text-sm font-medium text-green-800">Account Balance</span>
            </div>
...
export default function WhatsAppSettingsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (catalogRes.success) {
        const whatsappProviders = catalogRes.data.filter((p) => p.services?.includes('whatsapp'));
        setProviders(whatsappProviders);
      }

      if (connectedRes.success) {
        const connected = connectedRes.data.filter((p) => {
          const catalog = catalogRes.data?.find((c) => c.id === p.provider);
          return catalog?.services?.includes('whatsapp');
        });
  ...
  // Filter based on search query and sort alphabetically
  const filteredConnected = connectedProviders
    .filter(
      (p) =>
        !searchQuery ||
    ...
    .sort((a, b) => {
      const nameA = (a.name || a.provider || '').toLowerCase();
      const nameB = (b.name || b.provider || '').toLowerCase();
      return nameA.localeCompare(nameB);
    });

  const filteredAvailable = availableProviders
    .filter(
      (p) =>
        !searchQuery ||
        p.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const stats = {
    ...
    contacts: connectedProviders.reduce((sum, p) => sum + (p.stats?.contacts || 0), 0),
  };
