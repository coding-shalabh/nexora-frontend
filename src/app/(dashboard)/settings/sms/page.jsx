    ...
    <motion.div variants={itemVariants} className="bg-white rounded-2xl p-6 shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-semibold text-gray-900">{account.name || 'SMS Gateway'}</h3>
...
export default function SMSSettingsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (catalogRes.success) {
        const smsProviders = catalogRes.data.filter((p) => p.services?.includes('sms'));
        setProviders(smsProviders);
      }

      if (connectedRes.success) {
        const connected = connectedRes.data.filter((p) => {
          const catalog = catalogRes.data?.find((c) => c.id === p.provider);
          return catalog?.services?.includes('sms');
        });
  ...
  const stats = {
    ...
    templates: connectedProviders.reduce((sum, p) => sum + (p.templatesCount || 0), 0),
    messagesSent: connectedProviders.reduce((sum, p) => sum + (p.stats?.sent || 0), 0),
    delivered: connectedProviders.reduce((sum, p) => sum + (p.stats?.delivered || 0), 0),
  };
  ...
  return (
    <UnifiedLayout hubId="settings" pageTitle="SMS Settings" fixedMenu={null}>
      <motion.div
              ...
              {filteredConnected.length > 0 ? (
                ...
                >
                  ...
                  <h2 className="text-xl font-semibold mb-2 text-gray-900">
                    {searchQuery ? 'No providers found' : 'No SMS Provider Connected'}
                  </h2>
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    {searchQuery
                      ? 'Try adjusting your search'
                      : 'Connect an SMS gateway provider to start sending text messages.'}
                  </p>
                  {!searchQuery && (
                    <Button
                      size="lg"
                  ...
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {filteredAvailableProviders.map((provider) => (
                      <motion.div
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="bg-white rounded-xl p-4 border border-gray-200 hover:border-purple-300 cursor-pointer group transition-colors"
                        onClick={() => handleProviderClick(provider)}
                      >
                        <div className="flex items-start gap-3">
                          <div className="h-10 w-10 rounded-lg bg-purple-50 flex items-center justify-center text-lg font-bold text-purple-600 shrink-0">
                          ...
                          <ChevronRight className="h-5 w-5 text-gray-300 group-hover:text-purple-500 group-hover:translate-x-1 transition-all shrink-0" />
                        </div>
                        {provider.features && (
                        ...
                        {[
                          {
                            ...
                            desc: 'Select an SMS gateway provider',
                          },
                          {
                            step: '2',
                          ...
                          { step: '4', title: 'Start Messaging', desc: 'Send SMS from your inbox' },
                        ].map((item) => (
                          <div
                            key={item.step}
